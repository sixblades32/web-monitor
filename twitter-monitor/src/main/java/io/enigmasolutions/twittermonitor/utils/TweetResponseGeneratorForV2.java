...
public class TweetResponseGeneratorForV2 {
  ...
  public static TweetResponse generateV2(ArrayList<Tweet> tweets, ArrayList<User> users,
      TweetResponse.TweetResponseBuilder tweetResponseBuilder) {
    if (tweets.size() > 1) {
      tweets.forEach(tweet -> {
        ...
        if (tweet.getType() == TweetType.RETWEET) {
          ...
          if (tweet.isQuoteStatus()) {
            ...
            tweetResponseBuilder.quotedStatus(
                generateV2(retweetTweets, retweetUsers, TweetResponse.builder()));
            ...
            tweetResponseBuilder.retweetedStatus(
                generateV2(retweetTweets, retweetUsers, TweetResponse.builder()));
          }
        }
      });
      ...
      if (tweets.get(0).getType() == TweetType.REPLY) {
        ...
        tweetResponseBuilder.repliedStatus(
            generateV2(repliedTweetArray, repliedUserArray, TweetResponse.builder()));
      }
    }
