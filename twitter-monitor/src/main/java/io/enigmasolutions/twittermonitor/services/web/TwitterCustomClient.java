...
public class TwitterCustomClient {
    ...
    private ResponseEntity<V2Response> getV2ResponseEntity(
        ...
        String host = twitterScraper.getProxy().getHost();
        int port = Integer.parseInt(twitterScraper.getProxy().getPort());
        String username = twitterScraper.getProxy().getLogin();
        String password = twitterScraper.getProxy().getPassword();
